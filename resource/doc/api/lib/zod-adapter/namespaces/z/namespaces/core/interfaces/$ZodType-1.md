[**@beecode/msh-base-frame**](../../../../../../../README.md)

***

[@beecode/msh-base-frame](../../../../../../../README.md) / [lib/zod-adapter](../../../../../README.md) / [z](../../../README.md) / [core](../README.md) / $ZodType

# Interface: $ZodType\<O, I, Internals\>

Defined in: node\_modules/zod/v4/core/schemas.d.cts:97

## Extended by

- [`ZodType`](../../../interfaces/ZodType-1.md)
- [`_$ZodType`]($ZodType.md)
- [`$ZodStringFormat`]($ZodStringFormat.md)
- [`$ZodGUID`]($ZodGUID.md)
- [`$ZodUUID`]($ZodUUID.md)
- [`$ZodEmail`]($ZodEmail.md)
- [`$ZodURL`]($ZodURL.md)
- [`$ZodEmoji`]($ZodEmoji.md)
- [`$ZodNanoID`]($ZodNanoID.md)
- [`$ZodCUID`]($ZodCUID.md)
- [`$ZodCUID2`]($ZodCUID2.md)
- [`$ZodULID`]($ZodULID.md)
- [`$ZodXID`]($ZodXID.md)
- [`$ZodKSUID`]($ZodKSUID.md)
- [`$ZodISODateTime`]($ZodISODateTime.md)
- [`$ZodISODate`]($ZodISODate.md)
- [`$ZodISOTime`]($ZodISOTime.md)
- [`$ZodISODuration`]($ZodISODuration.md)
- [`$ZodIPv4`]($ZodIPv4.md)
- [`$ZodIPv6`]($ZodIPv6.md)
- [`$ZodMAC`]($ZodMAC.md)
- [`$ZodCIDRv4`]($ZodCIDRv4.md)
- [`$ZodCIDRv6`]($ZodCIDRv6.md)
- [`$ZodBase64`]($ZodBase64.md)
- [`$ZodBase64URL`]($ZodBase64URL.md)
- [`$ZodE164`]($ZodE164.md)
- [`$ZodJWT`]($ZodJWT.md)
- [`$ZodNumber`]($ZodNumber.md)
- [`$ZodNumberFormat`]($ZodNumberFormat.md)
- [`$ZodBoolean`]($ZodBoolean.md)
- [`$ZodBigInt`]($ZodBigInt.md)
- [`$ZodBigIntFormat`]($ZodBigIntFormat.md)
- [`$ZodSymbol`]($ZodSymbol.md)
- [`$ZodUndefined`]($ZodUndefined.md)
- [`$ZodNull`]($ZodNull.md)
- [`$ZodAny`]($ZodAny.md)
- [`$ZodUnknown`]($ZodUnknown.md)
- [`$ZodNever`]($ZodNever.md)
- [`$ZodVoid`]($ZodVoid.md)
- [`$ZodDate`]($ZodDate.md)
- [`$ZodArray`]($ZodArray.md)
- [`$ZodObject`]($ZodObject.md)
- [`$ZodUnion`]($ZodUnion.md)
- [`$ZodXor`]($ZodXor.md)
- [`$ZodDiscriminatedUnion`]($ZodDiscriminatedUnion.md)
- [`$ZodIntersection`]($ZodIntersection.md)
- [`$ZodTuple`]($ZodTuple.md)
- [`$ZodRecord`]($ZodRecord.md)
- [`$ZodMap`]($ZodMap.md)
- [`$ZodSet`]($ZodSet.md)
- [`$ZodEnum`]($ZodEnum.md)
- [`$ZodLiteral`]($ZodLiteral.md)
- [`$ZodFile`]($ZodFile.md)
- [`$ZodTransform`]($ZodTransform.md)
- [`$ZodOptional`]($ZodOptional.md)
- [`$ZodExactOptional`]($ZodExactOptional.md)
- [`$ZodNullable`]($ZodNullable.md)
- [`$ZodDefault`]($ZodDefault.md)
- [`$ZodPrefault`]($ZodPrefault.md)
- [`$ZodNonOptional`]($ZodNonOptional.md)
- [`$ZodSuccess`]($ZodSuccess.md)
- [`$ZodCatch`]($ZodCatch.md)
- [`$ZodNaN`]($ZodNaN.md)
- [`$ZodPipe`]($ZodPipe.md)
- [`$ZodCodec`]($ZodCodec.md)
- [`$ZodReadonly`]($ZodReadonly.md)
- [`$ZodTemplateLiteral`]($ZodTemplateLiteral.md)
- [`$ZodFunction`]($ZodFunction.md)
- [`$ZodPromise`]($ZodPromise.md)
- [`$ZodLazy`]($ZodLazy.md)
- [`$ZodCustom`]($ZodCustom.md)
- [`$ZodTypeDiscriminable`]($ZodTypeDiscriminable.md)

## Type Parameters

### O

`O` = `unknown`

### I

`I` = `unknown`

### Internals

`Internals` *extends* [`$ZodTypeInternals`]($ZodTypeInternals-1.md)\<`O`, `I`\> = [`$ZodTypeInternals`]($ZodTypeInternals-1.md)\<`O`, `I`\>

## Properties

### \_zod

> **\_zod**: `Internals`

Defined in: node\_modules/zod/v4/core/schemas.d.cts:98

***

### ~standard

> **~standard**: [`$ZodStandardSchema`](../type-aliases/$ZodStandardSchema.md)\<`$ZodType`\<`O`, `I`, `Internals`\>\>

Defined in: node\_modules/zod/v4/core/schemas.d.cts:99
